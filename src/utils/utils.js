 ...
 * @return Promise<{success: boolean, animeName: string|null}>
 */
      ...
      if (video) {
        ...
        video.addEventListener("timeupdate", () => {
          ...
          if (!hasTriggered && currentTime >= TIME_TO_DETECT && animeNameFromParent) { // Time subject to change
            hasTriggered = true; 
            alert("Requesting anime fetch for: " + animeNameFromParent);
            resolve({ success: true, animeName: animeNameFromParent });
          }
        });
      }
    })();
